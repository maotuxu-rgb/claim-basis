...
### 8. 可借鉴模块（来自 Agent Matrix 类框架 Wanman 的启示）

**背景**：开源项目 Wanman（`github.com/chekusu/wanman`，Apache-2.0）是本地多智能体编排框架——supervisor 协调多个**上下文隔离**的 Claude/Codex 子进程，配合**制品库 / 假设追踪 / 交叉验证 / 可观测复现**等模块，印证了 §7"枚举—检视—合成 + 独立复核"架构的工程可行性。**结论：学其模块设计与命名，不照搬其重型 supervisor/worktree/JSON-RPC 机制**（任务错位——它为自主写代码的 agent 团队设计，我们是线性法律推理）。以下 4 点可直接转化为本项目的 schema 与评估设计：

**① 制品库（Artifact）→ 把 JSON IR 升级为"可追溯制品"**
JSON 检索表不只是内存里临时传递的数据，而是有 ID、有版本、可落盘归档的"制品"——下游可读、评估脚本可比对。给 §7 schema 加一层"制品信封"：
```json
{
  "artifact_id": "anspruchsmatrix-<案名>-v1",
  "case_ref": "指导案例168号",
  "schema_version": "0.1",
  "produced_by": "enumerator",
  "请求权候选": [ ... ],
  "竞合": [ ... ]
}
```
收益：每案存一份"标准制品"，`test/` 可做自动 diff，直补 §3"无自动化"痛点。

**② 假设追踪（Hypothesis）→ 强化"争点 / 置信度"字段**
把 §7 schema 中过薄的 `争点[]`（字符串数组）升级为**结构化待证假设对象**，逼模型说清"取决于什么、谁裁断、若成立则如何"：
```json
"争点": [
  {
    "假设": "甲已施工拆改是否构成事实上履行不能",
    "依据法条": "第580条第1款第1项",
    "取决于": "事实认定",
    "由谁裁断": "法院",
    "若成立则": "司法解除路径②要件满足"
...
```
收益：把没把握处明确标为"假设"而非伪装成结论（呼应指导案例168号杜撰教训）。

**③ 交叉验证（Cross-validation）→ 复核 agent 的 rubric（`prompts/critic-rubric.md`，待制作）**
把 §7 误区1 的"独立复核"具体化为复核器审查清单，重点压在防幻觉：
```
你只能依据下方 JSON 和案情挑错，不得替它补全分析。逐项检查并产出结构化问题清单：
1. 【法条真实性】每个"规范基础"是否真实存在？条号/款项/措辞是否与 source 一致？
   —— 无法在 source 中核对的法条，标记「疑似杜撰」。
2. 【枚举完整性】是否漏列本应触发的请求权基础？(对照法律效果反查)
3. 【涵摄漏洞】每个"产生要件"的涵摄是否有事实支撑，还是空泛断言？
4. 【置信度虚高】结论确定但事实存疑的，置信度是否反而标了"高"？
5. 【竞合遗漏】成立的多个请求权之间，竞合关系是否处理？
输出：问题类型 / 定位到哪个候选 / 严重度（阻断 or 建议）。
```
收益：防幻觉从模型自觉变为专人专表强制核对，严于现单趟 skill。

**④ 可观测复现 → `test/` 的轻量自动比对**
不引入 JSON-RPC 重型机制，仅学"一切产物可被脚本读取"理念：评估时让流水线落盘 JSON 制品，写小脚本比对"实跑制品 vs 参考制品"的关键字段——
- 候选请求权的"规范基础"集合（漏列/多列？）
- 每个候选的"成立结论"（一致？）
- "争点"是否覆盖 ref 标注的核心争议？

收益：摆脱 §3 反复提及的"自评不可靠"，结构化字段可机器比对，不靠人肉读全文。

> **落地优先级**：②③可即刻并入 §7 schema 草案与新建 critic rubric（零成本）；①④待做指令版/SDK 版时一并上。**申报书定位**：点明已调研 Agent Matrix 类框架（Wanman），但本项目面向请求权基础分析这一垂直任务做**轻量专用流水线**，以领域知识资产深度为核心竞争力，区别于通用编排框架。

#### 落地形态：借鉴 Wanman 模式、裁剪为三至四个职能 agent

**定位澄清（勿对立）**：「借鉴 Wanman 模式」与「用 Claude Agent SDK 实现」**不是二选一**——Wanman 是成品应用（调度 Claude Code/Codex CLI 子进程），SDK 是写代码的工具箱；正确关系是**借 Wanman 的架构思想、用某种工具（SDK 或直接 API 调用）实现**。本项目的两阶段架构（§7）本质上**就是一个"裁剪到三四个 agent 的 Wanman"**。

**对照（通用 → 垂直瘦身）**：
| Wanman（通用·一堆 agent） | 本项目（垂直·3-4 个） |
|---|---|
| supervisor 总管 | 编排脚本（串流程） |
| CEO/开发/运维/营销… | 枚举器 / 复核器 / 渲染器 |
| 制品库 | JSON 检索表（§8①） |
| 交叉验证模块 | 复核器盲查（§8③） |

**三至四个职能 agent**：
```
① 枚举器   穷尽列候选请求权、逐一四步检视 → 输出 JSON 检索表
② 复核器   上下文隔离盲查 JSON：法条真伪 / 漏列 / 涵摄漏洞 / 置信度虚高（见 §8③ rubric）
③ 渲染器   只照定稿 JSON 写报告，不新增结论
④（可选）法源核验 agent  专查 source/annotated-code.md，为①引用的每条法条"验明正身"
            —— 第④个直击杜撰痛点（指导案例168号教训），从源头掐住幻觉
```

**关键边界——借"模式"放心，fork"它的代码"要谨慎**：
- ✅ **借模式/思想**：多智能体、supervisor 协调、上下文隔离、制品传递、可观测复现——随便借，是好架构常识。
- ⚠️ **fork 其代码当底座**：会继承一堆**用不上的包袱**——git worktree 隔离（为隔离"agent 改代码"而设；法律推理**不改文件**，worktree 对本项目无意义）、Node 运行时、SQLite、JSON-RPC。**借骨架设计，别扛整套机器**；自写几十行串起三四个 agent，比 fork 再删更干净。

**实现三档（保持开放、随时可停）**：
| 档 | 做法 | 代价 | 适用 |
|---|---|---|---|
| 0 指令版 | SKILL.md 内编排"先出 JSON→再渲染"，零代码 | 零 | 先验证想法值不值 |
| 1 轻量脚本 | 几十行，调模型 3-4 次、每次干净上下文（即"三四个 agent"瘦身版） | 低 | **当前最自然落点** |
| 2 完整框架 | fork Wanman 或自建 supervisor | 高 | 真要产品化 / 可观测复现 |
> 当前态度：保持包容、不预设偏向。推进路径建议 **先 0 档验证 → 有效则上 1 档**，2 档作远期可能性留白、不承诺。

**申报书措辞（不绑死实现）**：
> "本项目借鉴 Agent Matrix 类多智能体协作模式（如 Wanman），面向请求权基础分析任务**裁剪为枚举—复核—渲染（及可选的法源核验）三至四个职能智能体**的轻量流水线；**具体实现路径保持开放**，先以提示词编排验证增益，再视效果选择轻量脚本或框架化方案。"
